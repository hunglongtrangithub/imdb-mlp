...
for token_level in ["word_level", "char_level"]:
    ...
    char_level_trainer = IMDBTrainer(IMDBExperimentConfig(**token_level_trainer_config))
...
# Save the results
results_df.to_csv(
    Path(__file__).resolve().parents[2] / "reports" / "best_model_results.csv",
    index=False,
)
stats_df.to_csv(
    Path(__file__).resolve().parents[2] / "reports" / "best_model_stats.csv",
    index=False,
)
